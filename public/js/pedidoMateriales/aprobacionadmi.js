...
function lista_por_aprobar(){
    $.get('/materiales/lista_aprobaradmi',function(data){
...
function cargartablesolicitud(data){
    ...
    $.each(data['detalle'], function(i, item){
        ...
        if(item['estado_dep_soli']=='A' && item['estado_dep_admi']=='P'){
            ...
            if(item['observacion_bodega']!=null){
                var color='background-color:#ffb3b3';
                observacion=item['observacion_bodega'];
            }
    	...
    	$('#lista_pedidos').append(`<tr role="row" class="odd">
	                       ...
	                       <td width="10%" style=" vertical-align: middle; text-align:center"  class="paddingTR">
                            ...
                            <td width="5%" style=" ${color}; vertical-align: middle; text-align:center "  class="paddingTR">
                                ${observacion}
                            </td>
...
function vermemoadmi(){
    vistacargando('m','Por favor espere...');
    $.get(`/materiales/visualizardocamini/${$('#idsolicitud_encrypt_firmar').val()}`, function(docB64){
            vistacargando();
            $("#modal_metodo_firma").modal("hide");
            $("#modal_firma_electronica").modal("show");
            var encabezado = '<hr style="margin: 10px 0px;"><p style="font-weight: 700;"><i class="fa fa-desktop"></i> Vista previa del documento</p>';
            $("#doc_generado").html(encabezado+" "+`<iframe id="iframe_document" src="data:application/pdf;base64,${docB64}" type="application/pdf" frameborder="0" style="width: 100%; height: 650px;"></iframe>`);
    }).fail(function(){
        vistacargando();
        $("#doc_generado").html(`
...
$("#frm_firma_electronica").submit(function(e){ 
    ...
    $.ajax({
          url: "/materiales/firmar_solicitudadmi",
          method: 'POST',
          data: FrmData,
          dataType: 'json',
          contentType:false,

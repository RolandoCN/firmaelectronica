$('#frm_buscar').submit(function(e){
    ...
    $.ajax({
        type: "POST",
        url: '/gestionCoactiva/listadoEmisionNotificacion',
...
$.each(data['detalle'], function(i, item){
    ...
    if(item['generado']=='true'){
        ...
        var generado=`<td style=" text-align:center; vertical-align: middle;"  class="paddingTR">
                            ...
                            <a  type="button" onClick="verpdf_titulo('${item['nombredoc']}')" class="btn btn-info btn-xs"> <span class="fa fa-eye"></span> Ver</a>

                        </td>`;
        $('#lista_doc').append(`<input type="hidden" name="input_lista_doc_t[]" value='${item['nombredoc']}'>`);
    }else{
        var generado=`<td style=" text-align:center; vertical-align: middle;"  class="paddingTR">
    ...
    if(item['generado_boleta']=='true'){
        contadorGenerado++;
        var generado_boleta=`<td style=" text-align:center; vertical-align: middle;"  class="paddingTR">
                            <span style="color: #fff;  background-color: #28a745;" class="badge badge-success">${item['boletafechageneracion']}</span><br>
                            <div style="padding-top:10px"></div>
                            <a  type="button" onClick="verpdf_boleta('${item['boletanombredoc']}')" class="btn btn-info btn-xs"> <span class="fa fa-eye"></span> Ver</a>

                        </td>`;
        $('#lista_doc').append(`<input type="hidden" name="input_lista_doc[]" value='${item['boletanombredoc']}'>`);
    }else{
        var generado_boleta=`<td style=" text-align:center; vertical-align: middle;"  class="paddingTR">
                            <span class="badge badge-warning">No generado</span>
                        </td>`;
...
if(contadorGenerado>0){
    // $('#btnDesacargarReporte').css('display','inline');
...
// FUNCION PARA FIRMAR ELECTRONICAMENTE DOCUMENTO -------------------
    ...
    $("#frm_firma_electronica_titulos").submit(function(e){ 
        ...
        $.ajax({
            url: "/gestionCoactiva/generarBoleta",
            method: 'POST',
            data: FrmData,
            dataType: 'json',
            contentType:false,
            ...
            success: function(retorno){
                ...
                if(retorno['error']==false){
                    ...
                    $('#procesing').hide();
                    $('#seccion_generar').hide();
                    $('#cantGeneradas').html(retorno.cantidad_generados);
                    $('#cantXgenerar').html(retorno.cantidad_generar);
                    // $('#catn_emisiones_generadas').show();
                    $('#boleta_generada').show();
                    var ruta=retorno.archivo;
                    var iframe=$('#iframePdfB');
                    iframe.attr("src", "/gestionCoactiva/visualizardoc/"+ruta);   
                    $("#vinculoBol").attr("href", '/gestionCoactiva/descargarDoc/'+ruta);
...
function verpdf_titulo(ruta){
    $('#titulo_modal').html('Título de Crédito');
    var iframe=$('#iframePdf');
    iframe.attr("src", "/gestionCoactiva/visualizardoc/"+ruta);   
    $("#vinculo").attr("href", '/gestionCoactiva/descargarDoc/'+ruta);
    $("#documentopdf").modal("show");
}

function verpdf_boleta(ruta){
    $('#titulo_modal').html('Boleta de Notificacion');
    var iframe=$('#iframePdf');
    iframe.attr("src", "/gestionCoactiva/visualizarBoleta/"+ruta);   
    $("#vinculo").attr("href", '/gestionCoactiva/descargarBoleta/'+ruta);
    $("#documentopdf").modal("show");
}
...
$('#descargar_boleta').click(function(){
    $('#modal_firma_electronica').modal("hide");
});

$('#modal_firma_electronica').on('hidden.bs.modal', function (e) {
    var iframe=$('#iframePdfB');
    iframe.attr("src", null);
    $('#seccion_generar').show();
    $('#boleta_generada').hide();
...
$('#formReporte').submit(function(e){
    ...
    $.ajax({
        type: "POST",
        url: '/gestionCoactiva/reporteBoletasGeneradas',
        data: new FormData(this),
        contentType: false,
        cache: false,
        processData:false,
        success: function(data){
            ...
            if(data['error']==true){
                alertNotificar(data['detalle'], "error");
                $('#btnDesacargarReporte').html(`<span class="fa fa-download"></span>Descargar Boletas Generadas`); 
                $('#btnDesacargarReporte').prop('disabled',false); 
               
                // return;
            }
            if(data['error']==false){
                window.open("/gestionCoactiva/descargarReporBoleta/"+data['nombredoc']);

...
function lista_solicitudes(){
    $.get('/gestionCoactiva/listado',function(data){
...
function detalle(id){
    ...
    $.get('/gestionCoactiva/detalle/'+id, function (data){
        if(data['error']==true){
...
function aprobar(id){
    ...
    function(isConfirm) {
        ...
        if (isConfirm) { // si dice que si
            ...
            $.get('/gestionCoactiva/aprobar/'+id, function (data){
                if(data['error']==true){
...
function generar_documento(id){
    ...
    $('#btn_modal_doc').html('');
    vistacargando("m", "Espere por favor");
    $.get('/gestionCoactiva/generarDocumento/'+id,function(data){
        console.log(data)
        vistacargando();
        if(data.error==true){
...
function verpdf(ruta,codigo,estdo_firma){
    ...
    // $("#vinculo").attr("href", '/gestionCoactiva/descargarDoc/'+codigo+'/descargar');
    ...
    $('#btn_modal_doc').append(`<a href="/gestionCoactiva/descargarDoc/${codigo}/descargar" id="vinculo"><button  type="button" id="descargar"class="btn btn-primary"><i class="fa fa-mail"></i> Descargar</button> </a>        
    ${btn_subir_arch}
    ${btn_volver_generar} `)
}
...
$('#frmArchivoSubir').submit(function(e){
    ...
    $.ajaxSetup({
       ...
       $.ajax({
           type: "POST",
           url: '/gestionCoactiva/subirDocumento',
...
function volver_generar(id){
    ...
    function(isConfirm) {
        ...
        if (isConfirm) { // si dice que si
            ...
            vistacargando("m", "Espere por favor");
            $.get('/gestionCoactiva/volverGenerarDocumento/'+id,function(data){
                vistacargando();
                if(data.error==true){
                    alertNotificar(data.mensaje,'error');
                    return;                      
